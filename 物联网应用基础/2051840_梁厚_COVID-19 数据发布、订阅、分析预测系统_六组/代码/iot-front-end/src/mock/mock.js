...
function countDeviceNum() {
    const a = Mock.mock({
        success: true,
        data: {
            alarmNum: '@integer(10, 100)',
            offlineNum: '@integer(100, 2000)',
            onlineNum:'@integer(20, 100)',
            totalNum:'@integer(200, 500)'
        }
    })
    // console.log("疫情统计看这里：",a);
    return a
}
...
function centermap(options) {
    // console.log("options看这里：",options);
    let params = parameteUrl(options.url)
    // console.log("params看这里：",params);
...
function ranking() {
   //多生成几个避免重复 重复会报错
  let num =Mock.mock({"list|8":[{ value:"@integer(50,1000)",name:"@city()"}]}).list
//   console.log("num看这里：",num);
  let newNum =[],numObj={}
  num.map(item=>{
