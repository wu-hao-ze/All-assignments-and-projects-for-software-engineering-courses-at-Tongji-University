  ...
  <div id="app">
    <el-container style="margin-top: 10px">
      <el-header>
        <el-space alignment="flex-start">
          <a href="https://sse.tongji.edu.cn/">
            <el-image
              src="https://z3.ax1x.com/2021/06/04/2Y8EWV.jpg"
              style="width: 200px; float: left; position: relative; left: -420px"
            ></el-image>
          </a>
          <div style="position: relative; right: -420px">
            ...
            <el-link href="https://github.com/wu-hao-ze" target="前往github首页"
              >1953729 吴浩泽</el-link
            >
          </div>
        </el-space>
      ...
      <el-container style="height: 600px" direction="horizontal">
        ...
        >
          <Information
            :sum-instruction="sumInstruction"
            :replace-algorithm="replaceAlgorithm"
            :execution-sequence="executionSequence"
            :instruction-each-page="instructionEachPage"
...
export default {
  ...
  watch: {
    missingPage(newValue) {
      gsap.to(this.$data, {
        duration: 0.5,
        animatedPage: this.curInstruct == 0 ? 0 : (newValue / this.curInstruct) * 100,
      })
    },
  },
  computed: {
    ...
    restart() {
      this.$confirm('确认重置指令状态?', '提示', {
        ...
        .then(() => {
          ...
          this.$refs.instructionTable.tableData.splice(
            0,
            this.$refs.instructionTable.tableData.length
          )
    ...
    findPos() {
      ...
      if (this.skipState == 0) {
        if (this.lastPos == -1) {
          ...
          this.skipState = 1
          if (
            this.lastPos + 1 < this.sumInstruction &&
            !this.instruction[this.lastPos + 1].finished
          ) {
    ...
    instructInfo() {
      this.$alert(
        '在左侧设置好<b>页面置换算法</b>和<b>执行顺序</b>后，即可通过<b>单步执行</b>一次执行一条指令或者<b>连续执行</b>直到指令执行结束。' +
          ...
          '<span style="float: right">指导老师：张慧娟</span><br>',
        '项目说明',
        {
          confirmButtonText: '确定',
